...
export default function Button({
  ...
  const variants = {
    ...
    secondary: "bg-white border border-slate-300 text-slate-700 hover:bg-slate-50 focus:ring-slate-500 shadow-sm",
  ...
  const paddings = "px-4 py-2.5 text-sm";

  return (
    <button
      className={`${baseStyles} ${variants[variant]} ${widths} ${paddings} ${className}`}
